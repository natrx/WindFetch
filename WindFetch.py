...
class Waterbody:
    ...
    def fetch(self, directions, minor_directions=None, minor_interval=None):
        ...
        def fetch_single_dir(self, dir):
            ...
            array_inv_pad = padding(
                array_inv_rot, pad_width, -self.resolution, inverse=True
            )
    ...
    def masking(self, waterbody, fill_value=None):
        ...
        if fill_value is not None:
            array_masked = np.where(np.isnan(self.array), fill_value, self.array) * (
                waterbody.landwater * -1
            )
        else:
